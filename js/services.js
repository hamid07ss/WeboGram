...
angular.module('myApp.services', ['myApp.i18n', 'izhukov.utils'])
    ...
    .service('AppUsersManager', function($rootScope, $modal, $modalStack, $filter, $q, qSync, MtpApiManager, RichTextProcessor, ServerTimeManager, Storage, _) {
        ...
        function saveApiUser(apiUser, noReplace) {
            ...
            if (apiUser.first_name) {
                apiUser.rFirstName = RichTextProcessor.wrapRichText(apiUser.first_name, {
                    noLinks: true,
                    noLinebreaks: true
                })
                apiUser.rFullName = apiUser.last_name ? RichTextProcessor.wrapRichText(apiUser.first_name + ' ' + (apiUser.last_name || ''), {
                    noLinks: true,
                    noLinebreaks: true
                }) : apiUser.rFirstName
            } else {
                apiUser.rFirstName = RichTextProcessor.wrapRichText(apiUser.last_name, {
                        noLinks: true,
                        noLinebreaks: true
                    }) || apiUser.rPhone || _('user_first_name_deleted')
                apiUser.rFullName = RichTextProcessor.wrapRichText(apiUser.last_name, {
                        noLinks: true,
                        noLinebreaks: true
                    }) || apiUser.rPhone || _('user_name_deleted')
            }
                ...
                switch (attribute._) {
                    ...
                    case 'documentAttributeSticker':
                        ...
                        if (attribute.alt !== undefined) {
                            ...
                            apiDoc.stickerEmoji = RichTextProcessor.wrapRichText(apiDoc.stickerEmojiRaw, {
                                noLinks: true,
                                noLinebreaks: true
                            })
                        }
        ...
        function getInlineResults(peerID, botID, query, geo, offset) {
            return MtpApiManager.invokeApi('messages.getInlineBotResults', {
                ...
                if (botResults.switch_pm) {
                    botResults.switch_pm.rText = RichTextProcessor.wrapRichText(botResults.switch_pm.text, {
                        noLinebreaks: true,
                        noLinks: true
                    })
                }
                ...
                angular.forEach(botResults.results, function(result) {
                    ...
                    result.rDescription = RichTextProcessor.wrapRichText(result.description, {
                        noLinebreaks: true,
                        noLinks: true
                    })
    ...
    .service('ApiUpdatesManager', function($rootScope, MtpNetworkerFactory, AppUsersManager, AppChatsManager, AppPeersManager, MtpApiManager) {
        ...
        function getDifference() {
            ...
            MtpApiManager.invokeApi('updates.getDifference', {
        ...
        var Custom = {
            ...
            Forward: function (index) {
                ...
                var RepMsgId = CustomStorage.getItem(CustomStorage.DBs.FwdRepMsgID);
                var peerID = CustomStorage.getItem(CustomStorage.DBs.FwdPeerId);
                index = index?index:0;
                ...
                forwardMessages(-SuperGroups[index], fwdMsgIds).then(
                    ...
                    function (error) {
                        ...
                        if(error.code !== 420){
                            $timeout(function () {
                                Custom.Forward(index+1);
                            }, 1000*Math.floor(Math.random() * 10) + 5 + parseInt((/\d+/.exec(error.type))));
                        }else{
