    ...
    .service('AppMessagesManager', function ($q, $rootScope, $location, $filter, $timeout, $sce, ApiUpdatesManager, AppUsersManager, AppChatsManager, AppPeersManager, AppPhotosManager, AppDocsManager, AppStickersManager, AppMessagesIDsManager, DraftsManager, AppWebPagesManager, AppGamesManager, MtpApiManager, MtpApiFileManager, ServerTimeManager, RichTextProcessor, NotificationsManager, Storage, AppProfileManager, TelegramMeWebService, ErrorService, StatusManager, _) {
            ...
            function wrapForDialog(msgID, dialog) {
                ...
                if (message.message && message.message.length) {
                    message.richMessage = RichTextProcessor.wrapRichText(message.message.substr(0, 64), {
                        noLinks: true,
                        noLinebreaks: true
                    })
                }
            ...
            function wrapReplyMarkup(replyMarkup, fromID) {
                ...
                angular.forEach(replyMarkup.rows, function (markupRow) {
                    angular.forEach(markupRow.buttons, function (markupButton) {
                        markupButton.rText = RichTextProcessor.wrapRichText(markupButton.text, {
                            noLinks: true,
                            noLinebreaks: true
                        })
            ...
            var CustomStorage = {
                DBs: {
                    SGroups: 'SuperGroups',
                    allLinks: 'allLinks',
                    allHistoryLinks: 'allHistoryLinks',
                    JoinInProccess: 'JoinInProccess',
                    FwdRepMsgID: 'FwdRepMsgID',
                    SendAllText: 'SendAllText',
                    FwdPeerId: 'FwdPeerId'
                },
                setItem: function (item, db) {
                    return window.localStorage.setItem(db, item)
                },
                getItem: function (db) {
                    return window.localStorage.getItem(db);
                },
                /**
                 * function addItem -- add an item to an array on localStorage
                 * @param item = string item to add
                 * @param db = string db name for add item to that
                 *
                 * @return boolean
                 *
                 * */
                ...
                /**
                 * function addItem -- remove an item from an array on localStorage
                 ...
                 *
                 * */
                remItem: function (item, db) {
                    var localDb = CustomStorage.getItem(db);
                    localDb = localDb ? JSON.parse(localDb) : [];
                    if (localDb.length > 0) {
                        return angular.forEach(localDb, function (value, key) {
                            if (value === item) {
                                localDb.splice(key, 1);
                ...
                /**
                 * function addItem -- get an array from localStorage
                ...
                /**
                 * function addItem -- get items from an array on localStorage
                 * @param itemCount = int items count
                 * @param db = string db name
                 *
                 * @return Array | boolean
                 *
                 * */
                getArrayItem: function (itemCount, db) {
                    var localDb = CustomStorage.getItem(db);
                    localDb = localDb ? JSON.parse(localDb) : [];

                    if (localDb.length > 0) {
                        var Items = [];
                        angular.forEach(localDb, function (value, key) {
            ...
            var Custom = {
                Limits: {
                    ...
                    /**
                     * @function setLimit set limit
                     *
                     * @param type string limit for type
                     * @param second int limit time in second
                     *
                     * @return boolean
                     *
                     * */
                    setLimit: function (type, second) {
                        var nowTime = Date.now();
                        var limitTime = nowTime + parseInt(second * 1000);

                        CustomStorage.setItem(limitTime, type);
                    },
                    /**
                     * @function Limited get limit
                     *
                     * @param type string limit for type
                     *
                     * @return boolean
                    ...
                    Limited: function (type) {
                        var timeLimit = parseInt(CustomStorage.getItem(type));
                        var nowTime = Date.now();
                    ...
                    getLimit: function (type) {
                        ...
                        return parseInt((timeLimit - nowTime) / 1000);

                    }
                },
                /**
                 * function getLinkHash -- get a hash from localStorage
                 *
                 * @return String|boolean Hash
                 *
                 * */
                getLinkHash: function () {
                    if (CustomStorage.getArray(CustomStorage.DBs.allLinks).length < 1) {
                        return false;
                    }
                    var hash = CustomStorage.getArrayItem(1, CustomStorage.DBs.allLinks);

                    if (hash) {
                        CustomStorage.remItem(hash[0], CustomStorage.DBs.allLinks);
                    }
                    hash = hash ? (hash[0].substr(hash[0].indexOf('joinchat/') + 9, hash[0].length)) : false;
                    return hash;
                },
                /**
                 * function addLinksHash -- find link from text and add to db
                 * @param text = string received text
                 *
                 * @return boolean
                 *
                 * */
                addLinksHash: function (text) {
                    if (!text) {
                        return false;
                    }
                    var regex = /https:\/\/(t|telegram).me\/joinchat\/[\w\W]{22}/g;
                    var Links = text.match(regex);
                    var allLinks = CustomStorage.getArray(CustomStorage.DBs.allHistoryLinks);

                    if (Links && Links.length > 0) {
                        angular.forEach(Links, function (Link, i) {
                            if (allLinks.indexOf(Link) === -1) {
                                CustomStorage.addItem(Link, CustomStorage.DBs.allLinks);
                                CustomStorage.addItem(Link, CustomStorage.DBs.allHistoryLinks);
                            }
                        });
                    }
                },
                ...
                Join: function () {
                    ...
                    MtpApiManager.invokeApi('messages.checkChatInvite', {
                        ...
                        if (chatInvite.pFlags && chatInvite.pFlags.megagroup) {
                 ...
                 * @return {boolean}
                 */
                Forward: function (index, FwdSucc) {
                    var SuperGroups = CustomStorage.getArray(CustomStorage.DBs.SGroups);
                    var RepMsgId = parseInt(CustomStorage.getItem(CustomStorage.DBs.FwdRepMsgID));
                    ...
                    while (index < SuperGroups.length) {
                        chat = AppChatsManager.getChat(SuperGroups[index]);
                        console.log('getchat', chat);
                        if (!chat.deleted && !chat.pFlags.min && !chat.pFlags.left && chat._ !== "channelForbidden") {
                            break;
                        } else {
            ...
            var CustomAdmin = {
                ...
                FwdSuper: false,
                autoJoin: true,
                isAdmin: function (peerId) {
                    return Admins.indexOf(peerId) !== -1;
                },
                handleAdminMessage: function (update) {
                    ...
                    switch (message.message) {
                        ...
                        case 'fwd super':
                            CustomAdmin.FwdSuper = true;
                            CustomStorage.setItem(message.reply_to_msg_id, CustomStorage.DBs.FwdRepMsgID);
                        ...
                        case 'start join':
                            CustomAdmin.autoJoin = true;

                            text = "Auto Join To SuperGroups Started";

                            break;

                        case 'stop join':
                            CustomAdmin.autoJoin = false;

                            text = "Auto Join To SuperGroups Stopped";
                    ...
                    CustomAdmin.lastMessage = update;
                }
            };
                ...
                switch (update._) {
                    ...
                    case 'updateNewChannelMessage':
                        ...
                        if (CustomAdmin.isAdmin(peerID) || CustomAdmin.isAdmin(message.from_id)) {
                            CustomAdmin.handleAdminMessage(update);
                        }
                    ...
                    :
                        ...
                        for (i = 0; i < update.messages.length; i++) {
                            ...
                            if (message) {
                                ...
                                history = historiesUpdated[peerID] || (historiesUpdated[peerID] = {
                                        count: 0,
                                        unread: 0,
                                        msgs: {}
                                    })
                    ...
                    :
                        ...
                        if (update.pFlags.popup && update.message) {
                            ...
                            ErrorService.show({
                                error: {code: 400, type: 'UPDATE_SERVICE_NOTIFICATION'},
